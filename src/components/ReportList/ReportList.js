...
export default function ReportList({ reports, onCardClick, isLoading }) {
    return <List sx={REPORT_LIST_STYLES.list}>
        {reports.map((report) => {
...
function ReportCard({ articleTitle, source, articleDateInUserTimeZone, randomImageUrl, objectivityScore }) {
    ...
    return <Card sx={REPORT_CARD_STYLES.card}>
        ...
        <Typography sx={REPORT_CARD_STYLES.textLine} style={{ width: '100%', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
            <Box component="span" style={REPORT_CARD_STYLES.source}>{source}</Box>
            <Box component="span" style={REPORT_CARD_STYLES.articleDate}>{articleDateInUserTimeZone}</Box>
        </Typography>
        ...
        <Button variant='outlined' sx={REPORT_CARD_STYLES.viewReportButton}>{TEXTS.viewReport}</Button>
    </Card>;
}
...
function ReportCardSkeleton() {
    return <Card sx={REPORT_CARD_STYLES.card}>
        <Skeleton sx={{ ...REPORT_CARD_STYLES.textLine, ...REPORT_CARD_STYLES.articleTitle, width: 288 }} />
        <Typography sx={{ ...REPORT_CARD_STYLES.textLine, width: '100%', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
            <Skeleton component="span" sx={{ ...REPORT_CARD_STYLES.source, width: 80 }} />
            <Skeleton component="span" sx={{ ...REPORT_CARD_STYLES.articleDate, width: 56 }} />
        </Typography>
        <Box sx={REPORT_CARD_STYLES.image} />
        <Box sx={[REPORT_CARD_STYLES.objectivityScore]}>
            <Typography>
        ...
        <Button variant='outlined' disabled sx={REPORT_CARD_STYLES.viewReportButton}>{TEXTS.viewReport}</Button>
    </Card >;
}
